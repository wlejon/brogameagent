...
Game-agent algorithms library: navigation (grid A* + Recast/Detour polygon
navmesh), steering, ORCA local avoidance, perception, MCTS variants
(single-hero, decoupled, team, layered, options, info-set, root-parallel),
AlphaZero-style ExIt training, and hand-crafted autograd-free NN circuits, all
over a snapshot-restorable combat sim. MuZero and other model-based extensions
land here.
...
include/brogameagent/
  ...
  avoidance.h      AvoidanceSim: 2D ORCA with priorities, layers/masks,
                   elevation filtering; wired into World::tick()
  perception.h     LOS, FOV, canSee, aim / lead-aim
  ...
  *.h              World, Agent, Mcts variants (incl. info_set_mcts/belief/
                   observability, generic_mcts), observation/reward,
...
- `BROGAMEAGENT_WITH_NN` (default ON): the neural + learning layer. It is the
  *only* consumer of brotensor: `src/nn/*` and `src/learn/*` (plus
  `src/grid/harness.cpp`). The core (navigation, steering, avoidance,
  perception, MCTS, grid observation/recording) has zero brotensor references
  and never includes an `nn/` or `learn/` header. Keep it that way: with
  `-DBROGAMEAGENT_WITH_NN=OFF` the library builds with no brotensor dependency
  at all, which is what a minimal `bro` build wants. A GPU backend implies it.
- `BROGAMEAGENT_WITH_NAVMESH`: polygon navmesh via `recastnavigation`, the
  only external (non-sibling) dependency. Standalone builds default ON but
  soft-disable with a STATUS message when `find_package` can't see it;
  subdirectory consumers default OFF and must opt in. When on, brogameagent
  defines `BROGAMEAGENT_HAS_NAVMESH=1` on its interface. `nav_mesh.h` itself
  is Recast-free and ships unconditionally; only `src/nav_mesh.cpp` gates.
  Point CMake at it via the vcpkg toolchain or
  `CMAKE_PREFIX_PATH=<vcpkg>/installed/<triplet>`.

`brotensor` is an `add_subdirectory` dependency at `../brotensor` (added only
when the NN layer is on). `bromath` (header-only) is an unconditional sibling
...
recorder, …); it includes `nn/factored.h`, so it and the other NN test
binaries build only with `BROGAMEAGENT_WITH_NN=ON`. Core-only binaries that
build either way: `test_avoidance.cpp` (ORCA solver + `World` integration) and
`test_nav_mesh.cpp` (gated on `BROGAMEAGENT_WITH_NAVMESH`). NN-side extras:
`test_transformer.cpp` (MHA / FeedForward / TransformerBlock /
TransformerEncoder), `test_adam.cpp` (Adam bias-correction), and
`test_single_hero_net_tx.cpp` (SingleHeroNetTX forward / FD-gradient /
save-load). `tools/nn_check` runs finite-diff gradient checks over every
circuit. `tests/gpu/` is built only when a GPU backend is on and exercises the
dispatch layer (host↔device parity, batched ops, layer dispatch round-trips,
the MCTS / inference-server paths).

## Conventions

- **Core stays brotensor-free.** Navigation, steering, avoidance, perception,
  MCTS, and the grid observation/recording sources must not include `nn/` or
  `learn/` headers. That separation is what makes `BROGAMEAGENT_WITH_NN=OFF`
...
- **Third-party headers don't leak.** `nav_mesh.h` exposes no Recast/Detour
  types (pimpl + `bromath::Vec3`). New external-dependency features follow the
  same shape: clean header, gated `.cpp`.
- **Partial paths, not failures.** `NavGrid::findPath` / `NavMesh::findPath`
  clamp an unreachable goal to the closest reachable point; the `…Ex` variants
  report `partial` and can opt back into hard-fail. Don't add silent-empty
  failure paths.
- **Determinism.** Pathfinding, ORCA solves, and MCTS are deterministic for
  identical inputs (ORCA orders neighbors by `(distance, index)`; navmesh
  random points are seeded). Preserve that in new code; tests assert it.
...
- New navigation/steering feature: core-side, so no brotensor and no `nn/`
  include. Recast/Detour-backed work goes behind `BROGAMEAGENT_WITH_NAVMESH`.
