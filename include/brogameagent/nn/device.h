...
// by BROTENSOR_HAS_GPU.
//
// Calling to(Device::GPU) on a CPU-only build (BROTENSOR_HAS_GPU undefined) is
...
inline void device_require_cuda(const char* layer_name) {
    ...
    throw std::runtime_error(
        ...
        " to Device::GPU — built without BROTENSOR_HAS_GPU (CUDA support disabled)");
#else
    (void)layer_name;
#endif
}
